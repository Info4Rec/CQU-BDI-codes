...
class EncoderTextBERT(nn.Module):
    def __init__(self, config, order_embeddings=False, mean=True, post_transformer_layers=0):
        ...
        bert_config = BertConfig.from_pretrained(config['text-model']['bert-config'],
                                                 output_hidden_states=True,
                                                 num_hidden_layers=config['text-model']['extraction-hidden-layer'])
        bert_model = BertModel.from_pretrained(config['text-model']['bert-model'], config=bert_config)
        ...
        if not self.preextracted:
            self.tokenizer = BertTokenizer.from_pretrained(config['text-model']['bert-text'])
