...
def shard_dis(images, captions, model, shard_size=70, feat_lengths=None, lengths=None):
...
def shard_dis_reg(images, captions, model, shard_size=128, lengths=None):
    ...
    for i in range(len(images)):
        ...
        for j in range(n_cap_shard):
            ...
            s = Variable(torch.from_numpy(captions[cap_start:cap_end]), volatile=True).cuda()
            im = Variable(torch.from_numpy(images[i]), volatile=True).float().unsqueeze(0).expand(len(s), 3, 256,
                                                                                                  256).cuda()
