...
def set_save_path(opt):
	...
	'''
	opt.outf = opt.outf  +'_'+opt.data_name
...
def plot_loss_curve(opt, train_loss, val_loss, test_loss=None):
	...
	if test_loss:
		...
		fig, ax = plt.subplots()
		ax.plot(range(0, opt.SOLVER.MAX_ITERS*opt.Epoch), train_loss, label='Ranked loss')
		ax.plot(range(0, opt.SOLVER.MAX_ITERS*opt.Epoch), val_loss, label='feature loss')
		ax.plot(range(0, opt.SOLVER.MAX_ITERS*opt.Epoch), test_loss, label='  loss')
		legend = ax.legend(loc='upper right', fontsize='medium')
		plt.savefig(os.path.join(opt.outf, 'Loss_feat_indice_0329.png'), bbox_inches='tight')
		plt.show()
	else:
		train_loss = np.array(train_loss)
		val_loss = np.array(val_loss)
...
def plot_loss_curve_2(opt, train_loss):

	train_loss = np.array(train_loss)
	
	# Plot the loss curves
	fig, ax = plt.subplots()
	ax.plot(range(0, opt.SOLVER.MAX_ITERS), train_loss, label='rank loss')	
	legend = ax.legend(loc='upper right', fontsize='medium')
	plt.savefig(os.path.join(opt.outf, 'Loss_rank1105.png'), bbox_inches='tight')
	plt.show()

def plot_loss_curve_4(opt, train_loss, val_loss, val_loss2, test_loss):
	...
	# Plot the loss curves
	fig, ax = plt.subplots()
	ax.plot(range(0, opt.SOLVER.MAX_ITERS*5), train_loss, label='Simi loss')
	ax.plot(range(0, opt.SOLVER.MAX_ITERS*5), val_loss, label='feature loss')
	ax.plot(range(0, opt.SOLVER.MAX_ITERS*5), val_loss2, label='cluster loss')
	ax.plot(range(0, opt.SOLVER.MAX_ITERS*5), test_loss, label='total loss')
	legend = ax.legend(loc='upper right', fontsize='medium')
	plt.savefig(os.path.join(opt.outf, 'Loss_ml_all_loss.png'), bbox_inches='tight')
	plt.show()
