...
class BaseDataSet(Dataset):
    def __init__(self, img_source, transforms=None, mode="RGB"):
        ...
        self.true_index_dict = self._build_true_index_dict()
    ...
    def _load_data(self):
        with open(self.img_source, 'r') as f:
            for line in f:
                #print("line:", line)
                _path, _label , _true= re.split(r",| ", line.strip())
                self.path_list.append(_path)
                self.label_list.append(_label)
                self.true_list.append(_true)
    ...
    def _build_true_index_dict(self):
        index_dict = defaultdict(list)
        for j, true in enumerate(self.true_list):
            index_dict[true].append(j)
    ...
    def __getitem__(self, index):
        ...
        true_label = self.true_list[index]
